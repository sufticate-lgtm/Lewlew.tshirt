...
import { Trash2, Loader2, LogOut, Upload, ChevronDown, ChevronRight, Settings } from "lucide-react";
import {
  ...
  return(
    <div>
      ...
      <div className="xi-tabs">
        {[["orders","Đơn hàng"],["shirts","Màu áo"],["inks","Màu mực"],["designs","Hình in"],["settings","Cài đặt"]].map(([id,label])=>(
      ...
      {orders.map(o=>(
        <div key={o.code} className="xi-order-card">
          ...
          <div style={{marginTop:10,fontSize:13,color:"#6b675c"}}>
            ...
            <div>TT: {o.payment==="cod"?"COD":o.payment==="bank"?"CK":"Momo"}</div>
          </div>
        </div>
      ))}
    </div>
...
function ShirtsTab({password}){
  ...
  const [error,setError]=useState(null);
  const [editHex,setEditHex]=useState({}); // {id: hex}
  ...
  async function handleAdd(e){
    ...
    try{await adminAddShirtColor(password,name,hex,file);setName("");setFile(null);setPreviewUrl(null);load();}
    catch(e){setError(e.message);}finally{setBusy(false);}
  }

  async function handlePatchHex(id){
    const newHex=editHex[id];if(!newHex)return;
    try{await adminPatchShirtColor(password,id,{hex:newHex});load();}
  ...
  return(
    <div>
          ...
          <div key={c.id} style={{textAlign:"center",background:"#fff",border:"2px solid var(--line)",borderRadius:8,padding:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,marginTop:8,justifyContent:"center"}}>
              ...
              {editHex[c.id]&&editHex[c.id]!==c.hex&&(
                <button onClick={()=>handlePatchHex(c.id)} style={{fontSize:11,background:"var(--ink)",color:"#fff",border:"none",borderRadius:3,padding:"2px 6px",cursor:"pointer"}}>Lưu</button>
              )}
            </div>
          </div>
        ))}
      ...
      <form onSubmit={handleAdd}>
        ...
        {previewUrl && (
          <ColorEyedropper imageUrl={previewUrl} onColor={h=>setHex(h)} style={{marginBottom:12,maxWidth:480}}/>
        )}
        <br/>
...
function InksTab({password}){
  ...
  return(
    <div>
      <p className="xi-subtitle">Bảng màu mực này áp dụng cho tất cả hình in. Khách chọn màu từ đây cho từng layer.</p>
      {error&&<div className="xi-error">{error}</div>}
      <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:24}}>
        {list.map(c=>(
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:6,background:"#fff",border:"2px solid var(--line)",borderRadius:30,padding:"6px 12px 6px 8px"}}>
            ...
            <button onClick={async()=>{try{await adminDeleteInkColor(password,c.id);load();}catch(e){setError(e.message);}}}
              style={{background:"none",border:"none",cursor:"pointer",color:"#a8453a",padding:0,marginLeft:2}}><Trash2 size={14}/></button>
          </div>
        ))}
      </div>
...
function DesignsTab({password}){
  ...
  function load(){
    Promise.all([getDesigns(),getInkColors(),getSettings()])
      .then(([d,ic,s])=>{setDesigns(d);setInkColors(ic);setSettings(s);})
      .catch(e=>setError(e.message));
  }
  useEffect(load,[]);
  ...
  return(
    <div>
      <p className="xi-subtitle">
        Mỗi hình in gồm nhiều <strong>layer màu</strong> (mỗi layer là 1 file PNG đen trên nền trong suốt, hỗ trợ PNG/TIFF).
        Dùng công cụ định vị để đặt hình đúng vị trí trên thước đo.
      </p>
      {error&&<div className="xi-error">{error}</div>}
      ...
      <form onSubmit={handleAddDesign} style={{display:"flex",gap:10,maxWidth:460,marginTop:8}}>
        <input value={newName} onChange={e=>setNewName(e.target.value)}
          placeholder="Tên hình in mới, VD: Mèo Yoga" required
          style={{flex:1,border:"2px solid var(--ink)",borderRadius:3,padding:"10px 12px",fontFamily:"Work Sans",fontSize:14}}/>
        <button className="xi-btn-primary" type="submit">+ Hình in</button>
      </form>
    </div>
  );
}
...
function DesignCard({design,inkColors,settings,password,onChange,setError,expanded,onToggle}){
  ...
  const [newLayer,setNewLayer]=useState({name:"",defaultInkId:"",file:null});
  const [busy,setBusy]=useState(false);

  const rulerPhotoUrl = settings?.rulerMockupPhoto || settings?.rulerPhoto;
  ...
  async function savePosition(){
    setSavingPos(true);
    try{await adminPatchDesign(password,design.id,{printArea});onChange();}
    catch(e){setError(e.message);}finally{setSavingPos(false);}
  }

  async function handleAddLayer(e){
    e.preventDefault();if(!newLayer.file)return setError("Cần chọn file PNG.");
    setBusy(true);
    try{
      await adminAddLayer(password,design.id,newLayer.name,newLayer.defaultInkId||inkColors[0]?.id,newLayer.file);
      setNewLayer({name:"",defaultInkId:"",file:null});onChange();
    }catch(e){setError(e.message);}finally{setBusy(false);}
  }

  return(
    <div className="xi-order-card" style={{marginBottom:16}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={onToggle}>
        {expanded?<ChevronDown size={18}/>:<ChevronRight size={18}/>}
        <strong style={{fontSize:16,flex:1}}>{design.name}</strong>
        <div style={{display:"flex",gap:6}}>
          {design.layers.slice(0,3).map(l=>(
            <img key={l.id} src={l.png} alt={l.name}
              style={{width:32,height:32,objectFit:"contain",filter:"brightness(0)",background:"#f0f0f0",borderRadius:4,padding:2}}/>
          ))}
          {design.layers.length>3 && <span style={{fontSize:12,color:"#8a8576"}}>+{design.layers.length-3}</span>}
        </div>
        <button onClick={e=>{e.stopPropagation();if(!confirm("Xoá hình in này?"))return;
      ...
      {expanded&&(
        <div style={{marginTop:16,paddingTop:16,borderTop:"1px solid var(--line)"}}>
          ...
          <div style={{marginBottom:20}}>
            <span className="xi-label">Layers màu mực</span>
            {design.layers.length===0 && <p style={{fontSize:13,color:"#8a8576"}}>Chưa có layer nào. Thêm layer bên dưới.</p>}
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:16}}>
              {design.layers.map(l=>(
                <div key={l.id} style={{background:"#f8f5ef",border:"2px solid var(--line)",borderRadius:8,padding:10,textAlign:"center"}}>
                  <div style={{background:"#e8e5de",borderRadius:6,padding:8,marginBottom:8}}>
                    <img src={l.png} alt={l.name} style={{width:60,height:60,objectFit:"contain",filter:"brightness(0)"}}/>
                  </div>
                  <div style={{fontSize:12,fontWeight:700,marginBottom:4}}>{l.name}</div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4,marginBottom:6}}>
                    <div style={{width:12,height:12,borderRadius:"50%",background:inkColors.find(c=>c.id===l.defaultInkId)?.hex||"#888",border:"1px solid #ccc"}}/>
                    <span style={{fontSize:11,color:"#6b675c"}}>{inkColors.find(c=>c.id===l.defaultInkId)?.name||"—"}</span>
                  </div>
                  <select value={l.defaultInkId||""} style={{fontSize:11,width:"100%",marginBottom:6}}
                  ...
                  <button className="xi-remove-btn" style={{fontSize:11,margin:"0 auto",display:"flex",alignItems:"center",gap:3}}
                    onClick={async()=>{if(!confirm("Xoá layer này?"))return;try{await adminDeleteLayer(password,design.id,l.id);onChange();}catch(e){setError(e.message);}}}>
                    <Trash2 size={12}/> Xoá
                  </button>
                </div>
              ))}
            ...
            {/* Thêm layer */}
            <form onSubmit={handleAddLayer} style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end",background:"#f8f5ef",padding:12,borderRadius:8}}>
              <div className="xi-field" style={{flex:1,minWidth:160}}><label>Tên layer *</label>
                <input value={newLayer.name} onChange={e=>setNewLayer(l=>({...l,name:e.target.value}))} placeholder="VD: Thân mèo" required/>
              </div>
              <div className="xi-field"><label>Màu mặc định</label>
                <select value={newLayer.defaultInkId} onChange={e=>setNewLayer(l=>({...l,defaultInkId:e.target.value}))}>
                  <option value="">— Chọn —</option>
                  {inkColors.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label style={{display:"inline-flex",alignItems:"center",gap:6,cursor:"pointer",border:"2px dashed var(--line)",padding:"8px 12px",borderRadius:6,fontSize:13}}>
                  <Upload size={14}/>{newLayer.file?newLayer.file.name:"PNG / TIFF *"}
                  <input type="file" accept=".png,.tif,.tiff,image/png,image/tiff" style={{display:"none"}}
                    onChange={e=>setNewLayer(l=>({...l,file:e.target.files[0]}))}/>
                </label>
              </div>
              <button type="submit" className="xi-btn-secondary" disabled={busy}>
                {busy&&<Loader2 size={14} className="xi-spin"/>} + Thêm layer
              </button>
            </form>
          </div>
...
/* ── Cài đặt (ruler upload + calibration) ───────────────────── */
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:32}}>
        {[["rulerMockupPhoto","Ảnh thước đo trên áo (dùng để định vị hình in)"],
          ...
          <div key={field}>
            ...
            {settings[field]&&<img src={settings[field]} alt={field} style={{width:"100%",maxHeight:180,objectFit:"contain",borderRadius:6,border:"2px solid var(--line)",marginBottom:8}}/>}
            <label style={{display:"inline-flex",alignItems:"center",gap:6,cursor:"pointer",border:"2px dashed var(--line)",padding:"8px 12px",borderRadius:6,fontSize:13}}>
      ...
      {cal&&(
        <div style={{background:"#f8f5ef",borderRadius:8,padding:16,maxWidth:480}}>
          <span className="xi-label" style={{display:"flex",alignItems:"center",gap:6}}><Settings size={16}/> Hiệu chỉnh thước đo (calibration)</span>
          <p style={{fontSize:13,color:"#6b675c",marginBottom:14}}>
            Các giá trị này xác định vị trí điểm gốc (0,0) của thước đo trên ảnh mockup và tỉ lệ cm/pixel.
            Dựa vào ảnh <strong>MOCK_UP_CO_THUOC.jpg</strong> đã phân tích: origin ở 50.3% ngang, 26.1% dọc; 1cm = 1.35% chiều rộng ảnh.
          </p>
          {[["originX","Điểm gốc X (fraction 0-1)","0.503"],["originY","Điểm gốc Y (fraction 0-1)","0.261"],["fracPerCm","1 cm = ? fraction ngang ảnh","0.0135"]].map(([key,label,hint])=>(
            <div key={key} className="xi-field" style={{marginBottom:10}}>
              <label>{label} <span style={{fontWeight:400,color:"#8a8576"}}>({hint} hiện tại)</span></label>
              <input type="number" step="0.001" value={cal[key]||""} onChange={e=>setCal(c=>({...c,[key]:parseFloat(e.target.value)}))}
