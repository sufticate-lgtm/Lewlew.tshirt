...
function DesignCard({design,inkColors,settings,password,onChange,setError,expanded,onToggle}){
  ...
  const [pendingZones,  setPendingZones]  = useState(design.printZones?.length ? design.printZones : null);
  ...
  return(
    <div className="xi-order-card" style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={onToggle}>
        ...
        <select value={design.status||"public"}
          onClick={e=>e.stopPropagation()}
          onChange={async e=>{e.stopPropagation();try{await adminPatchDesignStatus(password,design.id,e.target.value);onChange();}catch(err){setError(err.message);}}}
          style={{fontSize:11,fontWeight:700,border:"2px solid var(--line)",borderRadius:4,padding:"2px 6px",
            background:design.status==="rd"?"#fef9c3":design.status==="private"?"#fee2e2":"#dcfce7",
            color:design.status==="rd"?"#92400e":design.status==="private"?"#991b1b":"#166534"}}>
      ...
      {expanded&&(
        <div style={{marginTop:16,paddingTop:16,borderTop:"1px solid var(--line)"}}>
          ...
          <div style={{marginBottom:24}}>
            <span className="xi-label" style={{marginBottom:8,display:"block"}}>📍 Vị trí in trên áo</span>
            <DesignPositioner
