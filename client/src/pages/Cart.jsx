  ...
  return (
    <div>
        ...
        {cart.map(item => (
          <div key={item.id} className="xi-cart-item">
            <div style={{width:56,height:56,borderRadius:6,overflow:"hidden",flexShrink:0,background:"#e8e5de"}}>
              {item.shirtPhoto &&
                <img src={item.shirtPhoto} alt={item.shirtName}
                  style={{width:"100%",height:"100%",objectFit:"cover"}}/>}
            </div>
            <div className="xi-cart-item-info">
              <div className="xi-cart-item-title">{item.designName} — Áo {item.shirtName}</div>
              <div className="xi-cart-item-meta" style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{width:10,height:10,borderRadius:"50%",background:item.inkHex,border:"1px solid #ccc",display:"inline-block"}}/>
                Mực {item.inkName} · Size {item.size} · {formatVND(item.unitPrice)}/cái
              </div>
            </div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:12,marginTop:20,flexWrap:"wrap"}}>
        <Link to="/" className="xi-btn-secondary"><ArrowLeft size={16}/> Tiếp tục chọn</Link>
